# Digital Abiogenesis Home

Obsidian-friendly navigation only; linked files remain authoritative for their subjects.

## Control Plane

- [[VISION|Vision and evidence standards]]
- [[PROJECT_STATE|Current project state]]
- [[ROADMAP|R0-R6 research roadmap]]
- [[NEXT_WORK_PACKET|Next approved planning target]]
- [[OPEN_QUESTIONS|Unresolved owner questions]]
- [[DECISIONS|Decision log]]
- [[VERIFICATION|Verification contract]]
- [[AGENTS|Codex collaboration constitution]]

## Engineering Reference

- [[docs/architecture/CURRENT_ARCHITECTURE|Current architecture]]
- [[docs/architecture/TECHNICAL_DEBT|Technical debt inventory]]
- [[docs/COMMANDS|Development and experiment commands]]
- [[docs/experiments/BACTERIUM_0_BENCHMARK_V1|Bacterium-0 benchmark v1]]
- [[docs/experiments/EXPERIMENT_ARTIFACT_CONTRACT|Experiment artifact contract]]
- [[docs/experiments/BACTERIUM_0_V1_BENCHMARK_REPORT|Frozen Bacterium-0 v1 benchmark report]]
- [[docs/experiments/examples/b0-quick-v1/summary|Artifact contract example]]
- [[docs/research/lenia/README|Lenia and neuroevolution research side track]]
- [[docs/research/NEAT_BACTERIUM0_FEASIBILITY|NEAT Bacterium-0 feasibility contract and result]]
- [[docs/work-packets/RS-02_CONWAY_TO_LENIA_CPU_REFERENCE|Proposed RS-02 CPU reference packet]]

## Historical Evidence

- [[docs/project-context|Original project context]]
- [[docs/lab-notes|Running lab notes]]
- [[docs/experiments|Experiment results]]
- [[docs/verification/README|Verification records]]
- [[docs/verification/2026-07-26-r1c|R1C canonical benchmark verification]]
